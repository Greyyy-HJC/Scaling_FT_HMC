...
from Scaling_FT_HMC.utils.field_trans_add_cos import FieldTransformation
from Scaling_FT_HMC.utils.func import hmc_summary, set_seed, plaq_from_field_batch, rect_from_field_batch
...
try:
    ...
    # print to check the weights of channels
    plaq_features = torch.ones([1, 2, lattice_size, lattice_size], device=device) # [batch_size, 2, L, L]
    rect_features = torch.ones([1, 4, lattice_size, lattice_size], device=device) # [batch_size, 4, L, L]
    
    for index in range(n_subsets):
        K0, K1 = nn_ft.models[index](plaq_features, rect_features) # [batch_size, 8, L, L], [batch_size, 16, L, L]
        
        K0_sq_avg = (K0**2).mean(dim=(0, 2, 3)) 
        K1_sq_avg = (K1**2).mean(dim=(0, 2, 3))
        
        print(f">>> K0 K1 check of subset {index}: ")
        print(f"K0 square average: {K0_sq_avg}")
        print(f"K1 square average: {K1_sq_avg}")
