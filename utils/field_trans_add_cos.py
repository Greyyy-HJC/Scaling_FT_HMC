...
class FieldTransformation:
    ...
    def compute_K0_K1(self, theta, index, plaq, rect):
        ...
        K0, K1 = self.models[index](plaq_features, rect_features) # [batch_size, 8, L, L], [batch_size, 16, L, L]
    ...
    def ft_phase(self, theta, index):
        ...
        K0, K1 = self.compute_K0_K1(theta, index, plaq, rect) # [batch_size, 8, L, L], [batch_size, 16, L, L]
        ...
        cos_rect_values = torch.cos(rect_angles)  # [batch_size, 8, L, L]
        
        # Apply signs according to the original calculation
        sin_rect_signs = torch.tensor([-1, 1, -1, 1, 1, -1, 1, -1], device=self.device, dtype=sin_rect_values.dtype)
        cos_rect_signs = torch.tensor([1, -1, 1, -1, -1, 1, -1, 1], device=self.device, dtype=cos_rect_values.dtype)
        # sin_rect_stack = sin_rect_values * sin_rect_signs.view(1, 8, 1, 1)
        rect_stack = torch.cat([sin_rect_values * sin_rect_signs.view(1, 8, 1, 1), cos_rect_values * cos_rect_signs.view(1, 8, 1, 1)], dim=1) # [batch_size, 16, L, L]
        
        temp = K1 * rect_stack
        ft_phase_rect = torch.stack([
            temp[:, 0] + temp[:, 1] + temp[:, 2] + temp[:, 3] + temp[:, 8] + temp[:, 9] + temp[:, 10] + temp[:, 11], # dir 0
            temp[:, 4] + temp[:, 5] + temp[:, 6] + temp[:, 7] + temp[:, 12] + temp[:, 13] + temp[:, 14] + temp[:, 15]  # dir 1
        ], dim=1)  # [batch_size, 2, L, L]
    ...
    def compute_jac_logdet(self, theta):
        ...
        for index in range(self.n_subsets):
            ...
            K0, K1 = self.compute_K0_K1(theta_curr, index, plaq, rect) # [batch_size, 8, L, L], [batch_size, 16, L, L]
            ...
            sin_rect_stack = -torch.sin(rect_angles)  # [batch_size, 8, L, L]
            rect_stack = torch.cat([cos_rect_stack, sin_rect_stack], dim=1) # [batch_size, 16, L, L]
            ...
            del temp, cos_rect_stack, sin_rect_stack, K1
