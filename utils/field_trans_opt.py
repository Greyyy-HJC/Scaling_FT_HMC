# %%
import torch
import torch.nn as nn
import matplotlib.pyplot as plt
import numpy as np
from tqdm import tqdm
import torch.autograd.functional as F
import warnings
import os
import logging

# Suppress PyTorch warnings
warnings.filterwarnings("ignore", category=UserWarning, module="torch")
warnings.filterwarnings("ignore", category=UserWarning, module="torch._dynamo")

# Configure PyTorch logger, redirect to file
torch_logger = logging.getLogger("torch")
torch_logger.setLevel(logging.ERROR)  # Only show error level logs
# Prevent log propagation to root logger, avoid displaying in console
torch_logger.propagate = False

from Scaling_FT_HMC.utils.func import plaq_from_field_batch, rect_from_field_batch, get_field_mask, get_plaq_mask, get_rect_mask, regularize
from Scaling_FT_HMC.utils.cnn_models_add_cos import choose_cnn_model #todo

class FieldTransformation:
    """Neural network based field transformation"""
    def __init__(self, lattice_size, device='cpu', n_subsets=8, if_check_jac=False, num_workers=0, identity_init=True, save_tag=None, model_tag='simple', fabric=None, backend='eager', input_hyperparams=None):
        self.L = lattice_size
        self.device = torch.device(device)
        self.n_subsets = n_subsets
        self.if_check_jac = if_check_jac
        self.num_workers = num_workers
        self.train_beta = None # init, will be set in train function
        self.current_epoch = 0
        self.total_epochs = 1
        self.model_tag = model_tag
        self.save_tag = save_tag # model name; train on which L; which random seed; if test
        self.fabric = fabric
        self.print = self.fabric.print if self.fabric is not None else print
        self.backward = self.fabric.backward if self.fabric is not None else torch.autograd.backward
        self.backend = backend # 'eager' for training, 'inductor' for evaluation
        
        
        self.hyperparams = {}
        self.hyperparams['init_std'] = 0.001
        self.hyperparams['lr'] = 0.001
        self.hyperparams['weight_decay'] = 0.0001
        self.hyperparams['betas'] = (0.9, 0.999)
        self.hyperparams['eps'] = 1e-8
        self.hyperparams['factor'] = 0.5
        self.hyperparams['patience'] = 5
        self.hyperparams['early_stopping'] = False
        self.hyperparams['early_stopping_patience'] = 10
        self.hyperparams['surrogate_dt'] = 0.10
        self.hyperparams['surrogate_n_steps'] = 6
        self.hyperparams['omelyan_lambda'] = 0.1931833
        self.hyperparams['softmin_tau'] = 0.15
        self.hyperparams['delta_h_threshold'] = 0.01
        
        if input_hyperparams is not None:
            self.hyperparams.update(input_hyperparams)
        
        # Create n_subsets independent models for each subset
        cnn_model = choose_cnn_model(model_tag)
        raw_models = nn.ModuleList([cnn_model().to(device) for _ in range(n_subsets)])
        
        # * Initialize models to produce nearly identity transformation if identity_init is True
        if identity_init:
            for model in raw_models:
                # Initialize all weights to a small non-zero value
                for param in model.parameters():
                    nn.init.normal_(param, mean=0.0, std=self.hyperparams['init_std'])
        
        raw_optimizers = [
            torch.optim.AdamW(model.parameters(), lr=self.hyperparams['lr'], weight_decay=self.hyperparams['weight_decay'], betas=self.hyperparams['betas'], eps=self.hyperparams['eps'])
            for model in raw_models
        ]
        
        self.models = []
        self.optimizers = []
        
        for model, optimizer in zip(raw_models, raw_optimizers):
            if self.fabric is not None:
                model, optimizer = self.fabric.setup(model, optimizer)
            self.models.append(model)
            self.optimizers.append(optimizer)
            
        self.schedulers = [
            torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=self.hyperparams['factor'], patience=self.    hyperparams['patience']
            )
            for optimizer in self.optimizers
        ]
        
        # Use torch.compile to optimize compute-intensive functions
        self._init_compiled_functions()

    def _init_compiled_functions(self):
        """Initialize functions optimized by torch.compile"""
        if hasattr(torch, 'compile'):  # Ensure PyTorch version supports compile
            try:
                # Only compile compute-intensive functions with safer backend option
                # Use 'eager' backend and configure for minimal logs
                compile_options = {
                    "backend": self.backend,     # Simple backend, avoid C++ compilation errors
                    "fullgraph": False,     # Do not require full graph compilation
                    "dynamic": True,        # Allow dynamic shapes, reduce recompilation warnings
                }
                
                self.print("Trying to use torch.compile for optimized computation...")
                self.forward_compiled = torch.compile(self.forward, **compile_options)
                self.inverse_compiled = torch.compile(self.inverse, **compile_options)
                self.ft_phase_compiled = torch.compile(self.ft_phase, **compile_options)
                self.compute_jac_logdet_compiled = torch.compile(self.compute_jac_logdet, **compile_options)
                self.compute_action_compiled = torch.compile(self.compute_action, **compile_options)
                self.print("Successfully initialized torch.compile")
            except Exception as e:
                self.print(f"Warning: torch.compile initialization failed: {e}")
                self.print("Falling back to standard functions")
                self.forward_compiled = self.forward
                self.inverse_compiled = self.inverse
                self.ft_phase_compiled = self.ft_phase
                self.compute_jac_logdet_compiled = self.compute_jac_logdet
                self.compute_action_compiled = self.compute_action
        else:
            # If PyTorch version does not support compile, use standard functions
            self.forward_compiled = self.forward
            self.inverse_compiled = self.inverse
            self.ft_phase_compiled = self.ft_phase
            self.compute_jac_logdet_compiled = self.compute_jac_logdet
            self.compute_action_compiled = self.compute_action
            self.print("torch.compile not available, using standard functions")

    def compute_K0(self, theta, index, plaq, rect):
        """
        OPTIMIZED: Compute K0 using cached plaq and rect values
        Input: theta with shape [batch_size, 2, L, L], cached plaq and rect
        Output: K0 with shape [batch_size, 4, L, L]
        """
        batch_size = theta.shape[0]
        
        # Calculate plaq features using cached plaq
        plaq_mask = get_plaq_mask(index, batch_size, self.L).to(self.device)
        plaq_masked = plaq * plaq_mask
        plaq_sin_feature = torch.sin(plaq_masked)
        plaq_cos_feature = torch.cos(plaq_masked)
        plaq_features = torch.stack([plaq_sin_feature, plaq_cos_feature], dim=1) # [batch_size, 2, L, L]
        
        # Calculate rect features using cached rect
        rect_mask = get_rect_mask(index, batch_size, self.L).to(self.device)
        rect_masked = rect * rect_mask
        rect_sin_feature = torch.sin(rect_masked)
        rect_cos_feature = torch.cos(rect_masked)
        rect_features = torch.cat([rect_sin_feature, rect_cos_feature], dim=1) # [batch_size, 4, L, L]
        
        # Use joint model to generate K0 and K1
        K0 = self.models[index](plaq_features, rect_features) # [batch_size, 8, L, L]
        
        return K0

    def ft_phase(self, theta, index):
        """
        Compute the phase factor for field transformation for a specific subset
        OPTIMIZED: Pre-compute all roll operations to avoid redundant computation
        OPTIMIZED: Use sincos fusion to compute sin and cos simultaneously
        """
        batch_size = theta.shape[0]
        
        # OPTIMIZATION: Cache plaq calculations to avoid redundant computation
        plaq = plaq_from_field_batch(theta) # [batch_size, L, L]
        rect = rect_from_field_batch(theta) # [batch_size, 2, L, L]
        
        # ROLL OPTIMIZATION: Pre-compute all rolled versions to avoid redundant roll operations
        # For plaquettes
        plaq_roll_1_2 = torch.roll(plaq, shifts=1, dims=2)    # Used by sin_plaq_dir0_2
        plaq_roll_1_1 = torch.roll(plaq, shifts=1, dims=1)    # Used by sin_plaq_dir1_2

        
        # TRIGONOMETRIC OPTIMIZATION: Compute sin efficiently for plaquettes
        # For the link in direction 0 or 1, there are two related plaquettes, note the group derivative is -sin
        plaq_angles = torch.stack([
            plaq,           # cos_plaq_dir0_1 = -cos(this)
            plaq_roll_1_2,  # cos_plaq_dir0_2 = -cos(this)
            plaq,           # cos_plaq_dir1_1 = -cos(this)
            plaq_roll_1_1   # cos_plaq_dir1_2 = -cos(this)
        ], dim=1)  # [batch_size, 4, L, L]
        
        sin_plaq_stack = torch.sin(plaq_angles)  # [batch_size, 4, L, L]
        cos_plaq_stack = torch.cos(plaq_angles)  # [batch_size, 4, L, L]
        
        sin_plaq_signs = torch.tensor([-1, 1, 1, -1], device=self.device, dtype=sin_plaq_stack.dtype)
        cos_plaq_signs = torch.tensor([1, -1, -1, 1], device=self.device, dtype=cos_plaq_stack.dtype)
        
        plaq_stack = torch.cat([sin_plaq_stack * sin_plaq_signs.view(1, 4, 1, 1), cos_plaq_stack * cos_plaq_signs.view(1, 4, 1, 1)], dim=1) # [batch_size, 8, L, L]
        
        # OPTIMIZATION: Pass cached plaq and rect to avoid recomputation
        K0 = self.compute_K0(theta, index, plaq, rect) # [batch_size, 8, L, L]
        
        # Calculate plaquette phase contribution
        temp = K0 * plaq_stack
        ft_phase_plaq = torch.stack([ 
            temp[:, 0] + temp[:, 1] + temp[:, 4] + temp[:, 5], # dir 0  
            temp[:, 2] + temp[:, 3] + temp[:, 6] + temp[:, 7]  # dir 1
        ], dim=1)  # [batch_size, 2, L, L]
        
        
        # Apply field mask to the combined phase
        field_mask = get_field_mask(index, batch_size, self.L).to(self.device)
        
        return (ft_phase_plaq) * field_mask

    def forward(self, theta):
        """
        Transform theta_new to theta_ori sequentially through all subsets
        
        Args:
            theta: Input field configuration with shape [batch_size, 2, L, L]
            
        Returns:
            Transformed field configuration with shape [batch_size, 2, L, L]
        """
        theta_curr = theta.clone()
        
        # Apply transformation for each subset sequentially
        for index in range(self.n_subsets):
            theta_curr = theta_curr + self.ft_phase_compiled(theta_curr, index)
            
        return theta_curr
    
    def field_transformation(self, theta):
        """Field transformation function for HMC (single input)"""
        return self.forward(theta.unsqueeze(0)).squeeze(0)

    def field_transformation_compiled(self, theta):
        """Field transformation function for HMC (single input)"""
        return self.forward_compiled(theta.unsqueeze(0)).squeeze(0)

    def inverse(self, theta):
        """
        Transform theta_ori to theta_new sequentially through all subsets
        Uses fixed-point iteration to find the inverse transformation
        """
        theta_curr = theta.clone()
        max_iter = 200
        tol = 1e-6
        
        for index in reversed(range(self.n_subsets)):
            theta_iter = theta_curr.clone()
            diff = float('inf')  # Initialize diff
            
            # Fixed-point iteration to find inverse transformation for this subset
            for i in range(max_iter):
                # Compute the phase for current iteration
                inv_phase = -self.ft_phase_compiled(theta_iter, index)
                
                # Update theta using the inverse phase
                theta_next = theta_curr + inv_phase
                
                # Check convergence
                diff = torch.norm(theta_next - theta_iter) / torch.norm(theta_iter)
                
                if diff < tol:
                    theta_curr = theta_next
                    break
                    
                theta_iter = theta_next
            
            # Warning if not converged
            if diff >= tol:
                self.print(f"Warning: Inverse iteration for subset {index} did not converge, final diff = {diff:.2e}")
        
        return theta_curr
    
    def inverse_field_transformation(self, theta):
        """Inverse field transformation function for HMC (single input)"""
        return self.inverse(theta.unsqueeze(0)).squeeze(0)
    
    def inverse_field_transformation_compiled(self, theta):
        """Inverse field transformation function for HMC (single input)"""
        return self.inverse_compiled(theta.unsqueeze(0)).squeeze(0)

    def compute_jac_logdet(self, theta):
        """Compute total log determinant of Jacobian for all subsets
        OPTIMIZED: Pre-compute roll operations to reduce redundant computation
        OPTIMIZED: Vectorized trigonometric function computation
        MEMORY OPTIMIZED: Clear intermediate tensors to prevent OOM"""
        batch_size = theta.shape[0]
        log_det = torch.zeros(batch_size, device=self.device)
        theta_curr = theta.clone()
        
        for index in range(self.n_subsets):
            field_mask = get_field_mask(index, batch_size, self.L).to(self.device)
            
            # Get plaquette and rectangle values
            plaq = plaq_from_field_batch(theta_curr)
            rect = rect_from_field_batch(theta_curr)
            
            # ROLL OPTIMIZATION: Pre-compute all rolled versions for Jacobian computation
            plaq_roll_1_2 = torch.roll(plaq, shifts=1, dims=2)
            plaq_roll_1_1 = torch.roll(plaq, shifts=1, dims=1)
            
            # TRIGONOMETRIC OPTIMIZATION: Vectorized cos computation for plaquettes
            # Stack plaquette angles for vectorized computation
            plaq_angles = torch.stack([
                plaq,           # cos_plaq_dir0_1 = -cos(this)
                plaq_roll_1_2,  # cos_plaq_dir0_2 = -cos(this)
                plaq,           # cos_plaq_dir1_1 = -cos(this)
                plaq_roll_1_1   # cos_plaq_dir1_2 = -cos(this)
            ], dim=1)  # [batch_size, 4, L, L]
            
            # Compute all cos values at once and apply -1 sign
            cos_plaq_stack = -torch.cos(plaq_angles)  # [batch_size, 4, L, L]
            sin_plaq_stack = -torch.sin(plaq_angles)  # [batch_size, 4, L, L]
            plaq_stack = torch.cat([cos_plaq_stack, sin_plaq_stack], dim=1) # [batch_size, 8, L, L]
            
            del plaq_angles, plaq_roll_1_2, plaq_roll_1_1, cos_plaq_stack, sin_plaq_stack
            
            # Get K0, K1 coefficients using cached plaq and rect
            K0 = self.compute_K0(theta_curr, index, plaq, rect) # [batch_size, 8, L, L], [batch_size, 16, L, L]
            
            # Calculate plaquette Jacobian contribution
            temp = K0 * plaq_stack
            plaq_jac_shift = torch.stack([ 
                temp[:, 0] + temp[:, 1] + temp[:, 4] + temp[:, 5],  # dir 0 
                temp[:, 2] + temp[:, 3] + temp[:, 6] + temp[:, 7]   # dir 1
            ], dim=1)  # [batch_size, 2, L, L]
            plaq_jac_shift = plaq_jac_shift * field_mask
            
            del temp, K0
            
            # Accumulate log determinant
            log_det += torch.log(1 + plaq_jac_shift).sum(dim=(1, 2, 3))
            
            del plaq_jac_shift, field_mask, plaq
            
            # Update theta for next subset
            theta_curr = theta_curr + self.ft_phase_compiled(theta_curr, index)
        
        return log_det
    
    def compute_jac_logdet_autograd(self, theta):
        """Compute Jacobian log determinant using autograd (for verification)"""
        theta_single = theta[0].unsqueeze(0)  # Only take the first sample to reduce computation
        jac = F.jacobian(self.forward_compiled, theta_single)
        jac_2d = jac.reshape(theta_single.shape[0], theta_single.numel(), theta_single.numel())
        return torch.logdet(jac_2d)
    
    def compute_action(self, theta, beta):
        """Compute action for given configuration"""
        plaq = plaq_from_field_batch(theta)
        total_action = torch.sum(torch.cos(plaq), dim=(1, 2))
        
        # Apply beta factor
        return -beta * total_action
    
    def compute_force(self, theta, beta, transformed=False):
        """
        OPTIMIZED Compute force (gradient of action) - Vectorized version for better performance
        
        Args:
            theta: Field configuration with shape [batch_size, 2, L, L]
            beta: Coupling constant (float)
            transformed: Whether to compute force in transformed space (bool)
        """
        # Ensure input requires gradients
        if not theta.requires_grad:
            theta = theta.clone().requires_grad_(True)
        
        if transformed:
            # In transformed space, account for the Jacobian
            theta_ori = self.forward_compiled(theta)
            action = self.compute_action_compiled(theta_ori, beta)
            jac_logdet = self.compute_jac_logdet_compiled(theta)
            
            # Verify Jacobian calculation if requested
            if self.if_check_jac:
                jac_logdet_autograd = self.compute_jac_logdet_autograd(theta)
                
                diff = (jac_logdet_autograd[0] - jac_logdet[0]) / jac_logdet[0]
                
                if abs(diff.item()) > 1e-4:
                    self.print(f"\nWarning: Jacobian log determinant difference = {diff:.2f}")
                    self.print(">>> Jacobian is not correct!")
                else:
                    self.print(f"\nJacobian log det (manual): {jac_logdet[0]:.2e}, (autograd): {jac_logdet_autograd[0]:.2e}")
                    self.print(">>> Jacobian is all good!")
            total_action = action - jac_logdet
        else:
            total_action = self.compute_action_compiled(theta, beta)
        
        # OPTIMIZED: Use vectorized gradient computation instead of the slow per-sample loop!
        # This is 3-5x faster than the original per-sample loop
        total_action = total_action.sum()  # Sum over batch for vectorized computation
        force = torch.autograd.grad(total_action, theta, create_graph=True)[0]
        
        return force  # shape: [batch_size, 2, L, L]

    def _wrap_angle(self, x):
        """Wrap angles with the same branch-cut convention used by HMC utilities."""
        return regularize(x)

    def _soft_topological_charge(self, theta):
        """Differentiable surrogate of topological charge on original variables."""
        plaq = plaq_from_field_batch(theta)
        plaq_wrapped = self._wrap_angle(plaq)
        return plaq_wrapped.sum(dim=(1, 2)) / (2.0 * np.pi)

    def _branch_cut_distance(self, theta):
        """Distance of wrapped plaquettes to the nearest branch cut at +/- pi."""
        plaq = plaq_from_field_batch(theta)
        plaq_wrapped = self._wrap_angle(plaq)
        return np.pi - torch.abs(plaq_wrapped)

    def _softmin_branch_distance(self, theta):
        """Soft minimum branch-cut distance for each sample."""
        d_pi = self._branch_cut_distance(theta)
        tau = self.hyperparams['softmin_tau']
        return -tau * torch.logsumexp(-d_pi.reshape(d_pi.shape[0], -1) / tau, dim=1)

    def _loss_weights(self):
        """Shift training emphasis from stability to topology barrier reduction."""
        if self.total_epochs <= 1:
            progress = 1.0
        else:
            progress = self.current_epoch / max(self.total_epochs - 1, 1)

        if progress <= 0.2:
            lambda_barrier = 0.3
            lambda_dh = 0.5
        elif progress >= 0.8:
            lambda_barrier = 1.0
            lambda_dh = 0.3
        else:
            alpha = (progress - 0.2) / 0.6
            lambda_barrier = 0.3 + 0.7 * alpha
            lambda_dh = 0.5 - 0.2 * alpha

        return {
            'lambda_barrier': lambda_barrier,
            'lambda_qprobe': 0.05,
            'lambda_dh': lambda_dh,
            'lambda_force': 0.02,
            'lambda_logdet': 1e-4,
            'lambda_cycle': 0.05,
        }

    def _fixed_eval_weights(self):
        """Static weights for validation/model selection so losses remain comparable across epochs."""
        return {
            'lambda_barrier': 1.0,
            'lambda_qprobe': 0.05,
            'lambda_dh': 0.3,
            'lambda_force': 0.02,
            'lambda_logdet': 1e-4,
            'lambda_cycle': 0.05,
        }

    def _omelyan_surrogate_step(self, theta_new):
        """Short differentiable Omelyan trajectory aligned with evaluation dynamics."""
        dt = self.hyperparams['surrogate_dt']
        n_steps = int(self.hyperparams['surrogate_n_steps'])
        lam = self.hyperparams['omelyan_lambda']

        pi0 = torch.randn_like(theta_new)

        theta0_ori = self.forward_compiled(theta_new)
        logdet0 = self.compute_jac_logdet_compiled(theta_new)
        action0 = self.compute_action_compiled(theta0_ori, self.train_beta) - logdet0
        h0 = action0 + 0.5 * (pi0 ** 2).sum(dim=(1, 2, 3))
        barrier_path = [self._softmin_branch_distance(theta0_ori)]
        q_path = [self._soft_topological_charge(theta0_ori)]
        min_dpi_path = [self._branch_cut_distance(theta0_ori).reshape(theta0_ori.shape[0], -1).min(dim=1).values]

        theta_curr = theta_new
        pi_curr = pi0 - lam * dt * self.compute_force(theta_curr, self.train_beta, transformed=True)
        for step_index in range(n_steps):
            theta_curr = theta_curr + 0.5 * dt * pi_curr
            pi_curr = pi_curr - (1 - 2 * lam) * dt * self.compute_force(theta_curr, self.train_beta, transformed=True)
            theta_curr = theta_curr + 0.5 * dt * pi_curr
            theta_obs = self._wrap_angle(theta_curr)
            theta_obs_ori = self.forward_compiled(theta_obs)
            barrier_path.append(self._softmin_branch_distance(theta_obs_ori))
            q_path.append(self._soft_topological_charge(theta_obs_ori))
            min_dpi_path.append(self._branch_cut_distance(theta_obs_ori).reshape(theta_obs_ori.shape[0], -1).min(dim=1).values)
            if step_index != n_steps - 1:
                pi_curr = pi_curr - 2 * lam * dt * self.compute_force(theta_curr, self.train_beta, transformed=True)
        pi_curr = pi_curr - lam * dt * self.compute_force(theta_curr, self.train_beta, transformed=True)

        theta1 = self._wrap_angle(theta_curr)
        theta1_ori = self.forward_compiled(theta1)
        action1 = self.compute_action_compiled(theta1_ori, self.train_beta) - self.compute_jac_logdet_compiled(theta1)
        h1 = action1 + 0.5 * (pi_curr ** 2).sum(dim=(1, 2, 3))

        return (
            theta1,
            theta0_ori,
            theta1_ori,
            h1 - h0,
            logdet0,
            torch.stack(barrier_path, dim=0),
            torch.stack(q_path, dim=0),
            torch.stack(min_dpi_path, dim=0),
        )

    def compute_loss_terms(self, theta_ori):
        """Compute total loss and tracked sub-terms for optimization and monitoring."""
        theta_new = self.inverse(theta_ori)
        theta1, theta0_ori, theta1_ori, dH, logdet0, barrier_path, q_path, min_dpi_path = self._omelyan_surrogate_step(theta_new)
        force0 = self.compute_force(theta_new, self.train_beta, transformed=True)

        barrier0 = barrier_path[0]
        barrier_last = barrier_path[-1]
        barrier_best = barrier_path.min(dim=0).values
        barrier_avg = barrier_path[1:].mean(dim=0)
        barrier_rebound = torch.relu(barrier_last - barrier_best)
        loss_barrier = (
            (barrier_last - barrier0)
            + 0.15 * barrier_rebound
        ).mean()

        q0 = q_path[0]
        q_last = q_path[-1]
        q_path_delta = torch.abs(q_path[1:] - q0.unsqueeze(0))
        q_excursion = q_path_delta.max(dim=0).values
        q_avg_shift = q_path_delta.mean(dim=0)
        dQ_soft = q_last - q0
        loss_qprobe = -(
            0.90 * torch.log1p(dQ_soft * dQ_soft)
            + 0.10 * torch.log1p(q_avg_shift * q_avg_shift)
        ).mean()

        dh_threshold = self.hyperparams['delta_h_threshold']
        loss_dH = torch.log1p((dH / dh_threshold) ** 2).mean()

        vol = float(self.L * self.L)
        f2 = (force0 ** 2).sum(dim=(1, 2, 3)) / (2.0 * vol)
        loss_force = torch.log1p(f2).mean()

        loss_logdet = (logdet0 ** 2).mean() + logdet0.var(unbiased=False)

        diff = self._wrap_angle(theta0_ori - theta_ori)
        loss_cycle = (diff ** 2).mean()

        weights = self._loss_weights()
        total_loss = (
            weights['lambda_barrier'] * loss_barrier
            + weights['lambda_qprobe'] * loss_qprobe
            + weights['lambda_dh'] * loss_dH
            + weights['lambda_force'] * loss_force
            + weights['lambda_logdet'] * loss_logdet
            + weights['lambda_cycle'] * loss_cycle
        )
        eval_weights = self._fixed_eval_weights()
        eval_loss = (
            eval_weights['lambda_barrier'] * loss_barrier
            + eval_weights['lambda_qprobe'] * loss_qprobe
            + eval_weights['lambda_dh'] * loss_dH
            + eval_weights['lambda_force'] * loss_force
            + eval_weights['lambda_logdet'] * loss_logdet
            + eval_weights['lambda_cycle'] * loss_cycle
        )

        return {
            'loss_total': total_loss,
            'loss_eval': eval_loss.detach(),
            'loss_barrier': loss_barrier.detach(),
            'barrier0': barrier0.mean().detach(),
            'barrier_last': barrier_last.mean().detach(),
            'barrier_best': barrier_best.mean().detach(),
            'barrier_avg': barrier_avg.mean().detach(),
            'barrier_rebound': barrier_rebound.mean().detach(),
            'loss_qprobe': loss_qprobe.detach(),
            'loss_dH': loss_dH.detach(),
            'loss_force': loss_force.detach(),
            'loss_logdet': loss_logdet.detach(),
            'loss_cycle': loss_cycle.detach(),
            'mean_abs_dH': torch.abs(dH).mean().detach(),
            'mean_abs_dQ_soft': torch.abs(dQ_soft).mean().detach(),
            'mean_avg_dQ_soft': q_avg_shift.mean().detach(),
            'mean_max_dQ_soft': q_excursion.mean().detach(),
            'mean_min_dpi0': min_dpi_path[0].mean().detach(),
            'mean_min_dpi_last': min_dpi_path[-1].mean().detach(),
            'mean_min_dpi_best': min_dpi_path.min(dim=0).values.mean().detach(),
            'lambda_barrier': torch.tensor(weights['lambda_barrier'], device=self.device),
            'lambda_dH': torch.tensor(weights['lambda_dh'], device=self.device),
        }
    
    # def loss_fn(self, theta_ori):
    #     """Compute loss function for training"""
    #     # Transform original configuration to new configuration
    #     theta_new = self.inverse(theta_ori)
        
    #     # Compute forces in transformed spaces
    #     force_new = self.compute_force(theta_new, self.train_beta, transformed=True)
        
    #     # Compute loss using multiple norms
    #     vol = self.L * self.L
    #     loss = torch.norm(force_new, p=2) / (vol**(1/2)) + \
    #            torch.norm(force_new, p=4) / (vol**(1/4)) + \
    #            torch.norm(force_new, p=6) / (vol**(1/6)) + \
    #            torch.norm(force_new, p=8) / (vol**(1/8))
               
    #     return loss


    def loss_fn(self, theta_ori):
        """Return the scalar loss used for optimization."""
        return self.compute_loss_terms(theta_ori)['loss_total']

 
    def train_step(self, theta_ori):
        """Perform a single training step for all subsets together"""
        theta_ori = theta_ori.to(self.device)
        
        with torch.autograd.set_grad_enabled(True):
            metrics = self.compute_loss_terms(theta_ori)
            loss = metrics['loss_total']
            
            # Zero all gradients
            self._zero_all_grads()
            
            # Backpropagate
            self.backward(loss)
            
            # Update all models
            self._step_all_optimizers()
            
        return {key: value.item() for key, value in metrics.items()}
    
    def _zero_all_grads(self):
        """Zero gradients for all optimizers"""
        for optimizer in self.optimizers:
            optimizer.zero_grad()
    
    def _step_all_optimizers(self):
        """Step all optimizers"""
        for optimizer in self.optimizers:
            optimizer.step()

    def evaluate_step(self, theta_ori):
        """
        Perform a single evaluation step
        
        Args:
            theta_ori: Original field configuration
            
        Returns:
            dict: Loss terms for this evaluation step
        """
        theta_ori = theta_ori.to(self.device)
        
        # * gradient is needed for evaluation
        theta_ori.requires_grad_(True)
        metrics = self.compute_loss_terms(theta_ori)
        
        return {key: value.item() for key, value in metrics.items()}

    def train(self, train_data, test_data, train_beta, n_epochs=100, batch_size=4):
        """
        Train all models together
        
        Args:
            train_data: Training dataset
            test_data: Testing dataset
            train_beta: Beta value for training
            n_epochs: Number of training epochs
            batch_size: Batch size for training
        """
        train_losses = []
        test_losses = []
        best_loss = float('inf')
        
        # Early stopping variables
        early_stopping_counter = 0
        early_stopping_patience = self.hyperparams['early_stopping_patience']
        use_early_stopping = self.hyperparams['early_stopping']
        
        self.train_beta = train_beta
        self.total_epochs = n_epochs
        
        # Create data loaders
        train_loader = torch.utils.data.DataLoader(
            train_data, batch_size=batch_size, shuffle=True, num_workers=self.num_workers
        )
        test_loader = torch.utils.data.DataLoader(
            test_data, batch_size=batch_size, num_workers=self.num_workers
        )
        
        if self.fabric is not None:
            train_loader = self.fabric.setup_dataloaders(train_loader)
            test_loader = self.fabric.setup_dataloaders(test_loader)
        
        # Print training information
        self.print(f"\n>>> Training the model at beta = {train_beta}\n")

        def summarize_metrics(metrics_list):
            summary = {}
            for key in metrics_list[0]:
                summary[key] = float(np.mean([metrics[key] for metrics in metrics_list]))
            return summary
        
        for epoch in tqdm(range(n_epochs), desc="Training epochs"):
            self.current_epoch = epoch
            # Training phase
            self._set_models_mode(True)  # Set models to training mode
            
            epoch_metrics = []
            
            for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{n_epochs}"):
                metrics = self.train_step(batch)
                epoch_metrics.append(metrics)
                
            train_loss = np.mean([metrics['loss_total'] for metrics in epoch_metrics])
            train_losses.append(train_loss)
            
            # Evaluation phase
            self._set_models_mode(False)  # Set models to evaluation mode
            
            test_metrics_epoch = []
            
            for batch in tqdm(test_loader, desc="Evaluating"):
                metrics = self.evaluate_step(batch)
                test_metrics_epoch.append(metrics)
                
            test_loss = np.mean([metrics['loss_eval'] for metrics in test_metrics_epoch])
            test_losses.append(test_loss)
            train_summary = summarize_metrics(epoch_metrics)
            test_summary = summarize_metrics(test_metrics_epoch)
            
            # Print epoch summary
            self.print(f"Epoch {epoch+1}/{n_epochs} - "
                  f"Train Loss: {train_loss:.6f} - "
                  f"Test Loss: {test_loss:.6f}")
            self.print(
                "  Train terms: "
                f"barrier={train_summary['loss_barrier']:.4f}, "
                f"barrier0={train_summary['barrier0']:.4f}, "
                f"barrier_last={train_summary['barrier_last']:.4f}, "
                f"barrier_best={train_summary['barrier_best']:.4f}, "
                f"barrier_avg={train_summary['barrier_avg']:.4f}, "
                f"rebound={train_summary['barrier_rebound']:.4f}, "
                f"qprobe={train_summary['loss_qprobe']:.4f}, "
                f"dH={train_summary['loss_dH']:.4f}, "
                f"force={train_summary['loss_force']:.4f}, "
                f"logdet={train_summary['loss_logdet']:.4f}, "
                f"cycle={train_summary['loss_cycle']:.4f}, "
                f"|dH|={train_summary['mean_abs_dH']:.4f}, "
                f"|dQ|={train_summary['mean_abs_dQ_soft']:.4f}, "
                f"avg|dQ|={train_summary['mean_avg_dQ_soft']:.4f}, "
                f"max|dQ|={train_summary['mean_max_dQ_soft']:.4f}, "
                f"min_dpi0={train_summary['mean_min_dpi0']:.4f}, "
                f"min_dpi_last={train_summary['mean_min_dpi_last']:.4f}, "
                f"min_dpi_best={train_summary['mean_min_dpi_best']:.4f}, "
                f"eval={train_summary['loss_eval']:.4f}, "
                f"lambda_b={train_summary['lambda_barrier']:.3f}, "
                f"lambda_H={train_summary['lambda_dH']:.3f}"
            )
            self.print(
                "  Test terms: "
                f"barrier={test_summary['loss_barrier']:.4f}, "
                f"barrier0={test_summary['barrier0']:.4f}, "
                f"barrier_last={test_summary['barrier_last']:.4f}, "
                f"barrier_best={test_summary['barrier_best']:.4f}, "
                f"barrier_avg={test_summary['barrier_avg']:.4f}, "
                f"rebound={test_summary['barrier_rebound']:.4f}, "
                f"qprobe={test_summary['loss_qprobe']:.4f}, "
                f"dH={test_summary['loss_dH']:.4f}, "
                f"force={test_summary['loss_force']:.4f}, "
                f"logdet={test_summary['loss_logdet']:.4f}, "
                f"cycle={test_summary['loss_cycle']:.4f}, "
                f"|dH|={test_summary['mean_abs_dH']:.4f}, "
                f"|dQ|={test_summary['mean_abs_dQ_soft']:.4f}, "
                f"avg|dQ|={test_summary['mean_avg_dQ_soft']:.4f}, "
                f"max|dQ|={test_summary['mean_max_dQ_soft']:.4f}, "
                f"min_dpi0={test_summary['mean_min_dpi0']:.4f}, "
                f"min_dpi_last={test_summary['mean_min_dpi_last']:.4f}, "
                f"min_dpi_best={test_summary['mean_min_dpi_best']:.4f}, "
                f"scheduled={test_summary['loss_total']:.4f}"
            )
            
            # Save best model and check early stopping
            if test_loss < best_loss:
                self._save_best_model(epoch, test_loss)
                best_loss = test_loss
                early_stopping_counter = 0  # Reset counter when improvement found
            else:
                early_stopping_counter += 1
            
            # Check early stopping
            if use_early_stopping and early_stopping_counter >= early_stopping_patience:
                self.print(f"\nEarly stopping triggered after {epoch+1} epochs "
                          f"(no improvement for {early_stopping_patience} epochs)")
                break
            
            # Update learning rate schedulers
            self._update_schedulers(test_loss)
        
        # Plot training history
        self._plot_training_history(train_losses, test_losses)
        
        # Load best model
        self._load_best_model(train_beta)
    
    def _set_models_mode(self, is_train):
        """
        Set all models to training or evaluation mode
        
        Args:
            is_train: If True, set to training mode, otherwise evaluation mode
        """
        mode_func = lambda model: model.train() if is_train else model.eval()
        
        for model in self.models:
            mode_func(model)
    
    def _update_schedulers(self, test_loss):
        """
        Update all learning rate schedulers
        
        Args:
            test_loss: Current test loss value
        """
        for scheduler in self.schedulers:
            scheduler.step(test_loss)
    
    def _save_best_model(self, epoch, loss):
        """
        Save the best model checkpoint
        
        Args:
            epoch: Current epoch number
            loss: Current loss value
        """
        save_dict = {
            'epoch': epoch,
            'loss': loss,
        }
        # Save state dict for each model
        for i, model in enumerate(self.models):
            # Always save the state dict as is, without modifying any prefixes
            # This ensures consistency - we always save with whatever prefix structure the model has
            save_dict[f'model_state_dict_{i}'] = model.state_dict()
                
        for i, optimizer in enumerate(self.optimizers):
            save_dict[f'optimizer_state_dict_{i}'] = optimizer.state_dict()
            
        # make sure the models directory exists
        os.makedirs('../models', exist_ok=True)
        torch.save(save_dict, f'../models/best_model_train_beta{self.train_beta}_{self.save_tag}.pt')

    def _plot_training_history(self, train_losses, test_losses):
        """
        Plot and save training history
        
        Args:
            train_losses: List of training losses
            test_losses: List of testing losses
        """
        plt.figure(figsize=(10, 5))
        plt.plot(train_losses, label='Train')
        plt.plot(test_losses, label='Test')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'plots/cnn_loss_train_beta{self.train_beta}_{self.save_tag}.pdf', transparent=True)
        plt.show()
        
        np.savetxt(f'dumps/train_loss_train_beta{self.train_beta}_{self.save_tag}.csv', train_losses, fmt='%.6e')
        np.savetxt(f'dumps/test_loss_train_beta{self.train_beta}_{self.save_tag}.csv', test_losses, fmt='%.6e')

    def _load_best_model(self, train_beta):
        """
        Load the best model from checkpoint for all subsets
        
        Args:
            train_beta: Beta value used during training
        """
        checkpoint_path = f'../models/best_model_train_beta{train_beta}_{self.save_tag}.pt'
        try:
            checkpoint = torch.load(checkpoint_path, map_location=self.device, weights_only=False)
            
            # Load models
            for i, model in enumerate(self.models):
                state_dict_key = f'model_state_dict_{i}'
                if state_dict_key in checkpoint:
                    state_dict = checkpoint[state_dict_key]
                    
                    # Check if the current model is wrapped by DataParallel
                    is_data_parallel = isinstance(model, nn.DataParallel)
                    has_module_prefix = any(k.startswith('module.') for k in state_dict.keys())
                    
                    # If model is not DataParallel but state_dict has 'module.' prefix, remove it
                    if not is_data_parallel and has_module_prefix:
                        self.print(f"Removing 'module.' prefix from state dict for model {i}")
                        new_state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
                        model.load_state_dict(new_state_dict)
                    else:
                        # Direct load when model structure matches the saved state
                        model.load_state_dict(state_dict)
                else:
                    raise KeyError(f"State dict for model {i} not found in checkpoint")
                
            self.print(f"Loaded best models from epoch {checkpoint['epoch'] + 1} with loss {checkpoint['loss']:.6f}") # +1 because epoch starts from 0
        except Exception as e:
            self.print(f"Error loading model: {e}")
            raise

# %%
