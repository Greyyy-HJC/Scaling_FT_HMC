...
class FieldTransformation:
    ...
    def compute_K0_K1(self, theta, index, plaq):
        """
        ...
        K0, K1 = self.models[index](plaq_features)
    ...
    def ft_phase(self, theta, index):
        ...
        # TODO
        # For the link in direction 0 or 1, there are two related plaquettes, note the group derivative is -sin
        sin_2plaq_dir0_1 = -torch.sin(2 * plaq) # [batch_size, L, L]
        sin_2plaq_dir0_2 = torch.sin(2 * plaq_roll_1_2)  # Use pre-computed roll
        
        sin_2plaq_dir1_1 = torch.sin(2 * plaq)
        sin_2plaq_dir1_2 = -torch.sin(2 * plaq_roll_1_1)  # Use pre-computed roll
        
        sin_2plaq_stack = torch.stack([sin_2plaq_dir0_1, sin_2plaq_dir0_2, sin_2plaq_dir1_1, sin_2plaq_dir1_2], dim=1) # [batch_size, 4, L, L]
        ...
        K0, K1 = self.compute_K0_K1(theta, index, plaq) # [batch_size, 4, L, L], [batch_size, 8, L, L]
        ...
        temp = K1 * sin_2plaq_stack
        ft_phase_2plaq = torch.stack([
            temp[:, 0] + temp[:, 1], # dir 0  
            temp[:, 2] + temp[:, 3]  # dir 1
        ], dim=1)  # [batch_size, 2, L, L]
        ...
        return (ft_phase_plaq + ft_phase_2plaq) * field_mask
    ...
    def compute_jac_logdet(self, theta):
        ...
        for index in range(self.n_subsets):
            ...
            cos_2plaq_stack = -torch.cos(2 * plaq_angles)  # [batch_size, 4, L, L]
            
            del plaq_angles, plaq_roll_1_2, plaq_roll_1_1
            
            # Get K0, K1 coefficients using cached plaq
            K0, K1 = self.compute_K0_K1(theta_curr, index, plaq) # [batch_size, 4, L, L], [batch_size, 8, L, L]
            ...
            plaq2_jac_shift = plaq2_jac_shift * field_mask
            
            del temp, cos_2plaq_stack, K1
            ...
            log_det += torch.log(1 + plaq_jac_shift + plaq2_jac_shift).sum(dim=(1, 2, 3))
            
            del plaq_jac_shift, plaq2_jac_shift, field_mask, plaq, rect, rect_dir0, rect_dir1
